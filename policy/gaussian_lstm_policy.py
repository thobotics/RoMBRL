...
class GaussianLSTMPolicy(StochasticPolicy, LayersPowered, Serializable):
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            flat_input_var_ = tf.placeholder(dtype=tf.float32, shape=(None, input_dim), name="flat_input")
            self.f_step_mean_std = tensor_utils.compile_function(*fetch_policy_out(flat_input_var_))

            self.custom_policy = fetch_policy_out
