...
from lib.utils.data_batches import generate_cluster_batches
...
class BayesNeuralNetDynModel(object):
    def __init__(self, session, n_inputs, n_outputs, tf_scope="default",
                 ...
                 batch_generator=generate_cluster_batches,
    ...
    def _initialize_variables(self, n_inputs, n_outputs, n_units):
        ...
        self.Y_Minibatch = tf.placeholder(shape=(None, n_outputs),
                                          dtype=self.dtype,
                                          name="Y_Minibatch")
        self.isTrain = tf.placeholder_with_default(True, shape=())
        ...
        with tf.variable_scope(self.tf_scope):
            ...
            self.log_R = tf.Variable(
                np.log(self.b0) * tf.ones([1, n_outputs]), dtype=self.dtype,
                name="log_R",  # trainable=False,
            )  # default 2.0; 0.1
            ...
            self.log_R = tf.clip_by_value(self.log_R, -20., 20.)
    ...
    def _create_optimizer(self, X, y, step_size, mdecay=0.05, cluster=None):
        ...
        if Sampler.is_burn_in_mcmc(self.sampling_method):
            ...
            # subclasses.
            self.sampler_kwargs.update({
                "scale_grad": self.scale / np.mean(self.norm_input.mean.eval()),  #n_datapoints,
                "mdecay": mdecay,
    ...
    def negative_log_likelihood(self, X, Y):
        ...
        f_mean = self.f_output
        # f_mean_1 = self.f_output + tf.random_normal(tf.shape(Y))
        ...
        def mixture_posterior(Y):
            ...
            return tf.reduce_logsumexp(
                [tf.log(weights[i]) + normldf_tf(y_diff[i], mu[i], var[i]) for i in range(len(mu))]
            )
        ...
        log_lik_data = tf.cond(self.isTrain, lambda: mixture_posterior(Y), lambda: normal_posterior(Y))
        log_posterior = tf.reduce_mean(log_lik_data)

        log_prior_data = (1 - self.a0) * tf.reduce_sum(log_Q) - self.b0 * tf.reduce_sum(inv_Q)
        log_prior_data += (1 - self.a0) * tf.reduce_sum(log_R) - self.b0 * tf.reduce_sum(inv_R)
        ...
        #
        # log_py_x = tf.concat([
        #     -0.5 * (tf.reduce_sum(tf.multiply(tf.multiply(y_diff, inv_Q), y_diff), axis=1) + tf.reduce_sum(log_Q,
        #                                                                                                    axis=1)),
        #     -0.5 * (tf.reduce_sum(tf.multiply(tf.multiply(y_diff_1, inv_R), y_diff_1), axis=1) + tf.reduce_sum(log_R,
        #                                                                                                    axis=1))
        # ], axis=0)
        #
        # Z = tf.concat([
        #     tf.ones([tf.shape(Y)[0], 1]) * np.log(0.01),
        #     tf.ones([tf.shape(Y)[0], 1]) * np.log(0.99)
        # ], axis=0)
        # logps_Z = gaussian_diag_logps(tf.zeros([tf.shape(Y)[0]*2, 1]),
        #                               tf.zeros([tf.shape(Y)[0]*2, 1]), Z)
        # logpz = tf.reduce_sum(logps_Z, axis=1)
        #
        # log_divergence = tf.reshape(log_py_x + logpz, [-1, 2])
        # logF = logsumexp(log_divergence)
        #
        # log_posterior = tf.reduce_mean(logF)
        #
        # log_prior_data = (1 - self.a0) * tf.reduce_sum(log_Q) - self.b0 * tf.reduce_sum(inv_Q)
        # log_prior_data += (1 - self.a0) * tf.reduce_sum(log_R) - self.b0 * tf.reduce_sum(inv_R)
        #
    ...
    def train_normal(self, X, y, X_val=None, y_val=None, cluster=None, step_size=1e-3, max_iters=8000):
        start_time = time()
        ...
        def log_full_training_error(iteration_index, total_nll, total_mse, is_sampling: bool):
        ...
        for j in range(0, max_iters + 1):
            ...
            _, training_loss, mse_loss = self.session.run([prediction_opt_op, self.Nll, self.Mse], batch_dict)
            if j % 500 == 0:
                log_full_training_error(j, training_loss, mse_loss, True)

        self.is_trained = True

    def train(self, X, y, X_val=None, y_val=None, cluster=None, step_size=1e-2, mdecay=0.05, burn_in_steps=1000, sample_steps=100, n_iters=50000, *args, **kwargs):
        ...
        self._create_optimizer(X, y, step_size=step_size, cluster=cluster)
        ...
        def log_full_training_error(iteration_index, is_sampling: bool):
            ...
            total_nll, total_mse = self.session.run(
                [self.Nll, self.Mse], feed_dict={
                    ...
                    self.Y_Minibatch: self.y,
                    self.isTrain: False
                    # self.cluster_Minibatch: np.ones_like(self.y, dtype=np.int32)
                }
            )
            ...
            total_nll_val, total_mse_val = self.session.run(
                [self.Nll, self.Mse], feed_dict={
                    ...
                    self.Y_Minibatch: y_val,
                    self.isTrain: False
                    # self.cluster_Minibatch: np.ones_like(y_val, dtype=np.int32)
                }
            )
    ...
    def compute_network_output(self, params, input_data):
        ...
        # feed_dict[self.cluster_Minibatch] = np.ones_like(input_data, dtype=np.int32)
