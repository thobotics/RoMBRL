...
BIG = 1e6
idx = 7
class BackwardSnakeEnv(MujocoEnv, Serializable):
    FILE = 'snake.xml'
    ORI_IND = 2
    ...
    def __init__(
        ...
        super(BackwardSnakeEnv, self).__init__(*args, **kwargs)
    ...
    def cost_np(self, x, u, x_next):
        ...
        return -np.mean(-x_next[:, idx] - self.ctrl_cost_coeff * 0.5 *np.sum(np.square(u), axis=1))

    def cost_tf(self, x, u, x_next):
        return -tf.reduce_mean(-x_next[:, idx] - self.ctrl_cost_coeff *0.5* tf.reduce_sum(tf.square(u), axis=1))
