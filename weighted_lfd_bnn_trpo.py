...
if __name__ == '__main__':
    ...
    # xu_training = xu_training[:n_timestep * 10 * 10]
    # y_training = y_training[:n_timestep * 10 * 10]
    # data_generator.set_offline(xu_training, y_training, xu_validate, y_validate)
    # training.add_data(xu_training[:, :n_states], xu_training[:, n_states:], y_training)

    """ Learning from demonstration """

    burn_in = [5000] * 10 + [5000] * 40
    ...
    #     training.run_normal(xu_va, y_va, step_size=1e-3, max_iters=5000)
    #     training.run_bnn(xu_va, y_va, step_size=10.0e-3, mdecay=0.05,
    #                      burn_in_steps=burn_in[itr], n_iters=100000, sample_steps=500)
    #
    ...
    #                              data_path=os.path.join(data_dir, 'lfd_bnn_trpo'))
    #
    #     """ Optimize Policy """
    ...
    burn_in = [3000]*10 + [3000]*40

    start_itr = 0

    for itr in range(start_itr, start_itr+30):  # 0, 25
        ...
        if itr == start_itr:
            training.run_normal(xu_va, y_va, step_size=1e-3, max_iters=3000)
            training.run_bnn(xu_va, y_va, step_size=10.0e-3, mdecay=0.05,
                             burn_in_steps=5000, n_iters=100000, sample_steps=200)
        else:
            training.run_normal(xu_va, y_va, xu_new, y_new, step_size=1e-3, max_iters=3000)
            training.run_bnn(xu_va, y_va, xu_new, y_new, step_size=10.0e-3, mdecay=0.05,
    ...
    for itr in range(start_itr, start_itr+30):
        data_generator.plot_traj(training, iter=itr, n_sample=10,
